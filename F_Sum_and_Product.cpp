...
void comderoP0612()
{
    ...
    while (q--)
    {
        ...
        // y = xai - ai^2;
        // ai^2 - xai + y = 0;
        // +-sqrt(x^2 - 4(1)(y))  // (-b +- sqrt(b^2-4*a*c))/2*a;
        // y/aj + aj = x;
        // y + aj^2 = xaj;
        // aj^2 - xaj + y = 0;
        // +-sqrt(y^2 - 4(1)(-x));
        ...
        {
            int num = x + (int)bssqrt(d);
            int num2 = x - (int)bssqrt(d);
            if (num % 2 == 0 and num2 % 2 == 0)
            {
                int root1 = ((num) / 2);
                int root2 = ((num2) / 2);
                bool it = (freq.find(root1) != freq.end());
                bool it2 = freq.find(root2) != freq.end();
                if (it2 == 1 and it == 1)
                {
                    if (root1 != root2)
                        cnt += (freq[root1] * freq[root2]);
                    else
                        cnt += ((freq[root1] * (freq[root2] - 1)) / 2);
                }
            }
        }
