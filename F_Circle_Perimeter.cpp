...
int pairs=0;
for(int x=0 ; x<=r ; x++){
    int low = sqrt(r*r-x*x);
    int high = sqrt((r+1)*(r+1) - x*x);
    if(x*x + high*high == (r+1)*(r+1)) high--;
    if(x*x + low*low < r*r) low++;
    pairs+=(high-low+1);
}
pairs-=1; //excluding pairs having x,y distance as r+1
pairs*=4; // we calculated only for positive quadrant we have such 4 so multiply by 4 done !!
// pairs+=4;
cout<<pairs<<endl;
