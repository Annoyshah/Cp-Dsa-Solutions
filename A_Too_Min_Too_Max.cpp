...
vector<int> minSubsequenceWithTargetSum(vector<int>& nums, int target) {
    int n = nums.size();
    unordered_map<int, int> dp; // dp[i] stores the length of the subsequence with sum i

    for (int num : nums) {
        for (auto it = dp.begin(); it != dp.end(); ++it) {
            int sum = it->first + num;
            int len = it->second + 1;
            if (sum <= target) {
                if (dp.find(sum) == dp.end() || len < dp[sum]) {
                    dp[sum] = len;
                }
            }
        }
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (dp.find(remainingSum - nums[i]) != dp.end() && dp[remainingSum - nums[i]] == dp[remainingSum] - 1) {
...
cin>>v;
sort(all(v));
int diff = abs(v[n-1]-v[0]) + abs(v[n-1]-v[1]) + abs(v[n-2]-v[1]) + abs(v[n-2]-v[0]);
cout<<diff<<endl;
}
int32_t main() { 
    // #ifndef ONLINE_JUDGE
